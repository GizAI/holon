...
#
# Usage example:
#   python holon_engine_v4.py --Lx 6 --Ly 4 --gamma Z6 --backend free --md out_v4.md --out out_v4.json
...
class MicroXY:
    ...
    def _apply_H_core(self, v: torch.Tensor):
        ...
        if self.twist_mode=="twist":
            ph_u = math.cos(self.phi/max(1,self.Lx))+1j*math.sin(self.phi/max(1,self.Lx))
        else:
            ph_u = None; cut_left=(self.wall_x-1)%max(1,self.Lx)
        for k,b in enumerate(self.bonds):
            ...
            if axis==0:
                if self.twist_mode=="twist":
                    ...
                    ph = (math.cos(self.phi)+1j*math.sin(self.phi)) if (xleft==cut_left) else (1.0+0j)
            else:
                ph=1.0+0j
            mi=self.mask_i[k]; mj=self.mask_j[k]
            states_flip = self.states ^ mi ^ mj
            vij = ((self.states & mi)!=0)&((self.states & mj)==0)
            vji = ((self.states & mj)!=0)&((self.states & mi)==0)
            amp_c = (-amp)
            out = out + amp_c*complex(ph)           * vij.to(v.dtype) * v.index_select(0, states_flip)
            out = out + amp_c*complex(ph).conjugate()* vji.to(v.dtype) * v.index_select(0, states_flip)
        return out
...
def one_particle_H_1d(Lx, t=1.0, phi=0.0, device=None, cdtype=torch.complex128):
    ...
    phase = math.cos(phi/max(1,Lx)) + 1j*math.sin(phi/max(1,Lx))
    a = float(torch.tensor(t, device=device, dtype=rdtype))
    for x in range(Lx):
        j = (x+1) % Lx
        ph = (phase if j==0 else 1.0+0j)
        H[x, j] += (-a)*complex(ph)
        H[j, x] += (-a)*complex(ph).conjugate()
    return H

def K_bare_1d_free(Lx, t=1.0, device=None, cdtype=torch.complex128):
    if device is None: device=_device()
    def E(phi):
        H = one_particle_H_1d(Lx, t=t, phi=phi, device=device, cdtype=cdtype)
        ev = torch.linalg.eigvalsh(H).real
        return float(ev.narrow(0, 0, Lx//2).sum().item())
    phi = 0.02
    E0 = E(0.0); Ep = E(+phi); Em = E(-phi)
    return float((Lx/(phi*phi)) * (Ep + Em - 2.0*E0))
...
def run_rge_two_loop_sm(a0: np.ndarray, mu0: float, mu1: float, steps=1200):
    """
    Adaptive RK2 with positivity & upper-bound guards to prevent overflow/NaN.
    """
    a = a0.astype(np.float64).copy()
    ...
    t = t0
    s = 0
    # integrate forward or backward depending on sign
    def advance_condition(t, t1, total_dt):
        return (t < t1 - 1e-15) if total_dt>0 else (t > t1 + 1e-15)

    while advance_condition(t, t1, total_dt):
        k1 = beta(a)
        a_mid = a + 0.5*dt*k1
        if (not np.all(np.isfinite(a_mid))) or np.any(a_mid <= 0) or np.any(a_mid > 0.5):
            dt *= 0.5
            if abs(dt) < 1e-8*abs(total_dt)/steps:
                return np.array([np.nan, np.nan, np.nan])
            continue
        k2 = beta(a_mid)
        a_new = a + dt*k2
        if (not np.all(np.isfinite(a_new))) or np.any(a_new <= 0) or np.any(a_new > 0.5):
            dt *= 0.5
            continue
        a = a_new
        t += dt
        s += 1
        if s % 50 == 0 and np.all(a < 0.15):
            dt *= 1.2
    return a
...
def predict_couplings_from_sigma(alpha_star_inv: float,
    ...
    aBL_ps = a4_ps  # simplest normalization (B-L from SU4 diagonal)
...
def flavor_pipeline(Lx, Ly, qmin: float):
    """
...
def run_engine(cfg: Config) -> Dict[str, Any]:
    ...
    # Z_K normalization from 1D bare
    K1d = K_bare_1d_free(cfg.Lx, t=cfg.t, device=device, cdtype=cdtype)
    ...
    return dict(
        ...
        K=dict(K_twist=K_tw, K_wall=K_w, K1d_bare=K1d, ZK=ZK, K_scaled=K_scaled),
        sigma=sigma,
...
def write_outputs(summary: Dict[str, Any], cfg: Config, wall_time: float):
    ...
    md.append("")
    md.append("## Checks")
    md.append(f"- Ward: divJ={summary['checks']['ward']['divJ']:.3e}, tol={summary['checks']['ward']['tol']:.1e}, passed={summary['checks']['ward']['passed']}")
    md.append(f"- OS positivity: min eig={summary['checks']['os']['min_eig']:.3e}, passed={summary['checks']['os']['passed']}")
...
def main():
    ...
    cfg=Config(Lx=args.Lx,Ly=args.Ly,Lz=args.Lz,beta=args.beta,flux_m=args.flux_m,gamma=args.gamma,
               ...
               dtype=args.dtype, backend=args.backend, mc_beta=args.mc_beta, mc_sweeps=args.mc_sweeps,

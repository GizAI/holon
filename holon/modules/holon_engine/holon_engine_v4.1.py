...
def _measure_elastic_backend(Lx, Ly, t_eff, backend, iters, m, device, cdtype):
    """
    free 백엔드: one_particle_H 기반 간단 곡률들
    """
    def E_free(phi=0.0, mode="twist", anis_eps=0.0, boundary_eta=0.0):
    ...
    # τ (wall)
    phi_w = 0.05
    E0w = E_free(0.0, "wall"); Ew = E_free(phi_w, "wall")
    tau = (Ew - E0w) / max(1, Ly)

    # κ (anisotropy curvature)
    eps = 1e-2
    Ep = E_free(0.0, "twist", anis_eps=+eps)
    Em = E_free(0.0, "twist", anis_eps=-eps)
    Eiso = E_free(0.0, "twist", anis_eps=0.0)
    kappa = ((Ep + Em - 2.0*Eiso) / (eps*eps)) / (Lx*Ly)

    # χ_b (boundary seam curvature)
    eta = eps
    Ebp = E_free(0.0, "twist", boundary_eta=+eta)
    Ebm = E_free(0.0, "twist", boundary_eta=-eta)
    chib = ((Ebp + Ebm - 2.0*Eiso) / (eta*eta)) / max(1, Ly)
    return float(tau), float(kappa), float(chib)

def hgc_rstar_and_ZK(Lx, Ly, t, backend, iters, m, device, cdtype):
    """
    r* 스캔을 통한 ZK 내추럴 정규화
    """
    import numpy as np
    ...
    for r in r_grid:
        ...
        # 간단 목적함수: λ1 - λ2 - λ3
        lam1 = (2.0 * Lx / ((2.0*math.pi)**2)) * tau
        lam2 = (1.0 / (Lx * Ly * Lx)) * kappa
        lam3 = (1.0 / (Lx * Ly)) * chib
        val = lam1 - lam2 - lam3
...
def run_engine(cfg: Config) -> Dict[str, Any]:
    ...
    if getattr(cfg, "zk_mode", "none") == "bare1d":
        ...
        hgc = hgc_rstar_and_ZK(cfg.Lx, cfg.Ly, cfg.t, cfg.backend, cfg.iters, cfg.subspace_m, device, cdtype)
        ZK = hgc["ZK"]

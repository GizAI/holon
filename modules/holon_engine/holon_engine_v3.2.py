...
def run_sm_two_loop(alpha_in, mu_in, mu_out, y_t=0.93, steps=4000):
    """
    안정적 2-loop 통합 (RK4). 알파가 비양/NaN 되면 즉시 중단.
    입력/출력: numpy array [a1, a2, a3]
    """
    b = sm_beta_1loop()
    B = sm_B_2loop()
    ...
    t0, t1 = math.log(mu_in), math.log(mu_out)
    dt = (t1 - t0) / steps
    ...
    def beta(a_vec):
        aa = a_vec * a_vec
        beta1 = -(b * inv2pi) * aa
        beta2 = -(inv8pi2) * aa * (B @ a_vec)
        return beta1 + beta2  # Yukawa feedback은 비활성화
    ...
    for _ in range(steps):
        ...
        # positivity & 폭주 guard
        if np.any(~np.isfinite(a)) or np.any(a <= 0) or np.any(a > 1.0):
            return np.array([np.nan, np.nan, np.nan], dtype=np.float64)
